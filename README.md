...
**前提（很重要）**：目标电脑上必须**安装过 cc-switch** —— 这个协议是它的安装程序注册的，
而且只写 `HKCU`（按用户，换个 Windows 账户就没了）。所以：

- 没装 cc-switch（或用的是没跑过安装程序的绿色版）→ 深链没有任何接收者，
  点了**不会有任何反应**（Windows/Chromium 在这种情况下都不会弹「选择应用」框）。
- 面板会把检测结果直接显示出来：`本机协议：已注册 ✓ → <cc-switch.exe 的实际路径>`，
  或 `未检测到 ccswitch:// 协议`。这是**每台电脑实测**的，不是写死的文字。
- 点「导入」时由**本机后端**（`POST /api/open-deeplink`）调用系统 ShellExecute 唤起，
  因此 exe 窗口模式和浏览器模式行为一致；唤不起来会**弹出说明框**并把深链复制到剪贴板，
  可以拿到已装 cc-switch 的电脑上，用 Win+R 或浏览器地址栏粘贴回车导入。
